...
import tw.jessie.sideproject.model.SearchRequest;
...
//	針對關鍵字搜尋到的專案進行日期排序
	...
	public String caseTimeDSC(Model model, HttpSession session) {
		...
		List<Order> caseTime = (List<Order>) session.getAttribute("keywordCase");

		keywordService.timeSortDSC(caseTime);
		for (Order order : caseTime) {
	...
	public String caseWant(Model model, HttpSession session) {
		...
		@SuppressWarnings("unchecked")
		List<Order> caseCollected = (List<Order>) session.getAttribute("keywordCase");

		// 針對搜尋到的專案獲取每個訂單的收藏人數
		for (Order order : caseCollected) {
